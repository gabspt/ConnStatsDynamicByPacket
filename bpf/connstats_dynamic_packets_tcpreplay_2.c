...
static inline int update_metrics(struct packet_t *pkt, struct global_metrics *globalm) {
    ...
    if (!initialized) {
        __u32 keyp = 0;
        __u32 *pkt_aggregation = bpf_map_lookup_elem(&userconfig, &keyp);
    ...
    if (flowmetrics != NULL) {
        ...
        // //Comprobar si hay que eliminar flujos
        ...
        //flow doesn't exist, create new flow if it is a new tcp syn or udp connection
